...
function initialize()
{
  ...
  $AWSCMD s3 cp "s3://$S3_BUCKET_NAME/ev_data.json" .
}
...
function stream_producer()
{
  ...
  python ~/ML-PowerFlow/App/DataStream/put-helper.py -s "$STREAM_NAME" -r "$AWS_REGION"
}
...
function stream_consumer()
{
  ...
  `python ~/ML-PowerFlow/App/DataStream/ev-properties-runner.py --print_command --java $(which java) --properties ~/ML-PowerFlow/App/DataStream/ev-consumer.properties`
...
function tear_down()
{
  ...
  $AWSCMD kinesis delete-stream --stream-name "$STREAM_NAME" --region "$AWS_REGION"
}
