...
class RecordProcessor(processor.RecordProcessorBase):
    ...
    * shutdown will be called if this MultiLangDaemon instance loses the lease to this shard, or the shard ends due
    ...
    def process_record(self, data, partition_key, sequence_number, sub_sequence_number):
        ...
        tags = {}
        tags['city'] = record['city']
        tags['zip'] = record['zip']
        tags['state'] = record['state']
        tags['country'] = record['country']
        tags['energy'] = record['energy']
	tags['stationID'] = record['stationID']
	tags['sessionID'] = record['sessionID']
        points = []
	for i in record['intervals']:
		print("ACCIO points:",i)
		points.append({
                    "measurement": "energy_consumed",
                    "tags": tags,
                    "time": i['stationTime'],
                    "fields": {
                        "energyConsumed": i['energyConsumed'],
                        "rollingPowerAvg": i['rollingPowerAvg'],
			"peakPower": i['peakPower'],
                    }
                })
	print("ACCIO POINTS: ", points)
        client.write_points(points)
        records = records + 1
        if records % 100 == 0:
            print("Records: %dLatest: %s" % (records, str(record['ts'])))
...
if __name__ == "__main__":
    client = InfluxDBClient('localhost', 8086, 'admin', 'admin', 'ev')
